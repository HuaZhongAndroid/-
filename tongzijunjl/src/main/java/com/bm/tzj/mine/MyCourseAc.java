...
public class MyCourseAc extends BaseActivity implements OnClickListener {
	...
	private void initView() {
		...
		dataList.add(new MyCourseListFrameLayout(context, "")); // 全部
		dataList.add(new MyCourseListFrameLayout(context, "001")); // 未开始
		dataList.add(new MyCourseListFrameLayout(context, "002")); // 进行中
		dataList.add(new MyCourseListFrameLayout(context, "003")); // 已结束
		dataList.add(new MyCourseListFrameLayout(context, "004")); // 待付款
...
//	private int oldSelect = 0;
	...
	private void selectTieleChange(int Index) {
		...
		case 0:
			...
			tv_total.setTextColor(getResources().getColor(
					R.color.app_dominantHue));
			break;
		case 1:
			v_2.setVisibility(View.VISIBLE);

			tv_notStart.setTextColor(getResources().getColor(
					R.color.app_dominantHue));
			break;
		case 2:
			v_3.setVisibility(View.VISIBLE);

			tv_underway.setTextColor(getResources().getColor(
					R.color.app_dominantHue));
			break;
		case 3:
			v_4.setVisibility(View.VISIBLE);

			tv_end.setTextColor(getResources()
					.getColor(R.color.app_dominantHue));
			break;
		case 4:
			v_5.setVisibility(View.VISIBLE);

			tv_payments.setTextColor(getResources().getColor(
					R.color.app_dominantHue));
			break;
		default:
			break;
