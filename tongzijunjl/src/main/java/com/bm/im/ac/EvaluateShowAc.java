...
public class EvaluateShowAc extends BaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setTitleName("查看评价");
		hotGoods = (SigninInfo) getIntent().getSerializableExtra("hotGoods");
	...
	private void loadDianping()
	{
		...
		map.put("coachid", GlobalParam.jiaolian);
		map.put("goodsid", hotGoods.goodsId);
		map.put("babyid", hotGoods.babyId);
		UserManager.getInstance().getCoachCommentByBaby(this, map, new ServiceCallback<CommonResult<CoachComment>>(){
			@Override
	...
	private void loadEvaluate()
	{
		...
		map.put("evaluateuserid", hotGoods.userId);
		map.put("babyid", hotGoods.babyId);
		UserManager.getInstance().getCommentByUser(this, map, new ServiceCallback<CommonListResult<EvaluateContent>>(){
			@Override
			public void done(int what, CommonListResult<EvaluateContent> obj) {
				if(obj.data != null && obj.data.size() > 0)
				{
					...
					tv_pingjia_t.setVisibility(View.GONE);
					tv_pingjia_r.setVisibility(View.GONE);
				}
			}
			@Override
			public void error(String msg) {
