...
public class ImApi {
	 ...
	 */
	  ...
	  * @param user
	  */
	 public static void syncUserInfo(CoachInfo user){
			String strUserId ="";
		 	if(!TextUtils.isEmpty(user.coachId)){
		 		strUserId=user.coachId;
...
//		    System.out.println("海盐"+muser.getUsername()+","+muser.getNick()+","+muser.getAvatar());
		...
		public static void syncGroupUserInfo(final Context context,final String groupId) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						 EMGroup returnGroup  = EMGroupManager.getInstance().getGroupFromServer(groupId);//根据群聊ID从服务器获取群聊基本信息
						 EMGroup  eMGroup=  EMGroupManager.getInstance().createOrUpdateLocalGroup(returnGroup);//保存获取下来的群聊基本信息
		...
		public static void getAppSys(Context context,String  userId,final Handler handler){
			...
			IMService.getInstance().getImUserInfoList(context,map, new ServiceCallback<CommonListResult<CoachInfo>>() {
				@Override
